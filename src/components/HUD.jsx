...
export default function HUD({ onOpenSettings }) {
  ...
  const { playerSlot, opponentJoined, clearRoom, lobbyState } = useMultiplayerStore()
  ...
  // Don't show full HUD while in online lobby
  const inOnlineLobby = isOnline && (lobbyState === 'idle' || lobbyState === 'creating' || lobbyState === 'joining' || lobbyState === 'waiting')
  ...
  // For online games that are in progress
  const isOnlineWaiting = isOnline && !opponentJoined
  ...
  return (
    <>
      <header className="header">
        ...
        {!gameOver && !inOnlineLobby && (
          <div className="turn-indicator">
            <div className={`piece-preview player-${currentPlayer} ${isBotThinking || isOnlineWaiting || isOpponentTurn ? 'thinking' : ''}`} />
            <span>{getTurnLabel()}</span>
          </div>
        )}
        
        {isOnline && opponentJoined && !gameOver && (
          <div className="online-player-info">
            You are <span className={`player-badge player-${playerSlot}`}>Player {playerSlot + 1}</span>
          </div>
        )}
        ...
        <div className="header-buttons">
          ...
          {isOnline && opponentJoined ? (
            <button className="leave-btn" onClick={handleLeaveOnline}>
              Leave Game
            </button>
          ) : showPlayAgainInHUD ? (
        ...
        <div className="game-settings">
          <div className="mode-selector">
            <button 
              ...
              disabled={isOnline && opponentJoined}
            >
            ...
            <button 
              className={`mode-btn ${gameMode === GAME_MODES.ONLINE ? 'active' : ''}`}
              onClick={() => setGameMode(GAME_MODES.ONLINE)}
              disabled={isOnline && opponentJoined}
            >
              <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round">
